...
var events = require('jslibs/winston/thirdPartyDeps/eventemitter2/eventemitter2'),
    ...
    vertxConsole = require('vertx/console');
...
Console.prototype.log = function (level, msg, meta, callback) {
  ...
  if (level === 'error' || level === 'debug') {
    vertxConsole.error(output);
  } else {
    vertxConsole.log(output);
  }

  // Event to enable the "stream" method below
  self.emit('written', output);
...
// Super simple "stream" method to echo the output; this should return the data
// actually "stored" by the transport
Console.prototype.stream = function (options) {
    var self = this,
        stream = new Stream;
    
    self.on('written', function (data) {
